...
function clockOut() {
  ...
  return getCard().then((timecard) => {
    let spot = getSpotForDay(timecard.card),
        spot_ends = spot && spot.times.every((s) => s.end)
    ...
    if (spot && !spot_ends) {
      ...
      throw new Error("There aren't any currently open times that can be closed.");
    } else {
      throw new Error("You never clocked in!");
    }
  }).then(setCard).then(() => {
    return {
      day: day,
