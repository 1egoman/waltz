...
function touchDotTimewaltz() {
  fs.mkdirpSync(`${process.env.HOME}/.timewaltz/templates`);
}
...
function getReportTemplate(filename) {
  return new Promise((resolve, reject) => {
    ...
    if (filename.endsWith(".ejs")) {
      ...
      return getReportTemplate(`${process.env.HOME}/.timewaltz/templates/${filename}.ejs`).then(resolve).catch(reject);
    }
  });
}
...
function showVersion() {
  console.log(`Timewaltz version ${chalk.green(pkg.version)}`);
}
